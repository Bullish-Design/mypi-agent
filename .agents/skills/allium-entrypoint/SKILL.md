...
**All `.allium` files in this project live under: `.scratch/specs`**

- Read specs from this directory when you need to understand existing behaviour.
- Write new specs to this directory.
- The entrypoint file is `.scratch/specs/index.allium` (create it if it doesn't exist).
...
3. **"I want to understand what a spec says"** — read the `.allium` files in `.scratch/specs`, use the `allium` skill for syntax help
...
```bash
allium check ".scratch/specs/index.allium"
```

Before implementation or test generation, also run:

```bash
allium analyse ".scratch/specs/index.allium"
```
